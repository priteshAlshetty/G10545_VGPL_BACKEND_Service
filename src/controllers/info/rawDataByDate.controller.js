...
async function getRawDataLatest(params) {
    const { meter_db, meter_id, date } = params;
    ...
    try {
        ...
        const result = await rawDb.query(rawDataLatestQuery, [date]);
...
async function getRawDataAvg(params) {
    ...
    WHERE DATE(date_time) = $1;
            `;
        const result = await rawDb.query(avgRawDataQuery, [date]);

        const rows = result.rows;
        const row = result.rows[0];
        if (Object.values(row).every(v => v === null)) {
            console.warn(`No avg raw data found for the specified meter ${meter_id} and date: ${date}`);
            return null;
        }
        return rows;
...
async function getRawDataContinuous(params) {
    ...
    ORDER BY date_time ASC;
        ...
        const result = await rawDb.query(continuousRawDataQuery, [date]);
...
async function getRawMeterDataByDate(params) {
    try {
        ...
        console.error('Error in getRawMeterDataByDate:', error);
        throw error;
    }
}
...
module.exports = { getRawMeterDataByDate };